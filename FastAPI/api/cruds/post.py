...
async def get_post(db: AsyncSession, user_id: int, post_id: int) -> Optional[model.Post]:
    result: Result = await db.execute(
        select(model.Post).filter(model.Post.user_id == user_id, model.Post.post_id == post_id)
    )
